...
**Revision history:** An earlier version of D13 (2026-04-21 morning) required an `-mvp` suffix on every file in `src/lib/`. That version was rewritten the same day after the project was renamed from "Plan Executor MVP" to "Plan Executor Tools" and the suffix's supposed collision-prevention role was re-examined and found to be redundant with directory-level disambiguation (`~/projects/dev/dev-tools/tools-dev/plan-executor-tools-dev/src/lib/`). This current version is the locked decision.
